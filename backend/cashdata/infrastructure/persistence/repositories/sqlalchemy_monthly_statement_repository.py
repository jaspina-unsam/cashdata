...
from cashdata.infrastructure.persistence.mappers.monthly_statement_mapper import (
...
class SQLAlchemyMonthlyStatementRepository(IMonthlyStatementRepository):
    ...
    def save(self, statement: MonthlyStatement) -> MonthlyStatement:
        ...
        if statement.id is None:
            ...
            model.billing_close_date = statement.billing_close_date
            model.payment_due_date = statement.payment_due_date
            self._session.flush()
            return MonthlyStatementMapper.to_entity(model)
